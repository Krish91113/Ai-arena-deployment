          ...
          <ReactMarkdown
            ...
            components={{
              ...
              code: ({ inline, children, ...props }: any) => 
                inline ? (
                  <code className="bg-secondary/70 px-1 py-0.5 rounded text-xs font-mono" {...props}>
                    {children}
                  </code>
                ) : (
                  <code className="block bg-secondary/70 p-2 rounded text-xs font-mono overflow-x-auto mb-3" {...props}>
                    {children}
                  </code>
                ),
          ...
          <div className="rounded-md bg-secondary/70 p-3 ring-1 ring-border/60 animate-card-pop">
            <div className="prose prose-sm dark:prose-invert max-w-none">
              <ReactMarkdown
                remarkPlugins={[remarkGfm]}
                components={{
                  p: ({ children }) => <p className="font-mono text-xs text-card-foreground/80 leading-relaxed mb-2">{children}</p>,
                  code: ({ inline, children, ...props }: any) => 
                    inline ? (
                      <code className="bg-background/30 px-1 py-0.5 rounded text-xs font-mono" {...props}>
                        {children}
                      </code>
                    ) : (
                      <code className="block bg-background/30 p-2 rounded text-xs font-mono overflow-x-auto mb-2" {...props}>
                        {children}
                      </code>
                    ),
                }}
