...
export default function PlaygroundPage() {
  ...
  const handleSubmit = async (question: string) => {
    ...
    try {
      const data = await askQuestion(question, user?.uid);
      setCurrentStep(4);
      setResponse(data);
    } catch (err) {
      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-2 md:grid-cols-4">
        {modes.map((m) => (
          <button
            ...
            className={`group relative overflow-hidden rounded-xl border px-4 py-3 text-left transition-all hover:-translate-y-1 ${mode === m.id ? 'border-primary bg-primary/10 shadow-lg shadow-primary/20' : 'border-border bg-card'
              }`}
          >
